  ...
  * Course: CS211
  * Quiz #15
  * Purpose: A. isPerfectMirror(); returns true if this list is a mirror from mirror().
  * B. undoMirror(); a void method that will undo the mirror() operation only if this starts mirrored, otherwise (!isPerfectMirror()) the undoMirror method leaves this unchanged..
 */ 
...
public class ArrayIntList implements Iterable<Integer> {
 ...
 // YOUR CODE GOES HERE
	...
	// write a method called isPerfectMirror to check if the list is a mirror
	public boolean isPerfectMirror() {
		...
		for (int i = 0; i < size; i++){
			if(elementData[i] != elementData[size - i - 1]){
				result = false;
			}
		}
		return result;

	} // closes isPerfectMirror method
	
	
	// write method undoMirror to undo the mirror and return its original list
	public void undoMirror() {
		if(isPerfectMirror() == true){
			int n = size / 2;
			for(int i = size - 1; i >= n; i--){
				remove(i);
			}
		}
	} // closes undoMirror method
